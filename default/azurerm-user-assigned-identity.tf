...
# cluster-2
#
resource "azurerm_role_assignment" "aks-mi-roles-aks-2" {
  scope                = azurerm_virtual_network.aks-2-vnet.id
  role_definition_name = "Network Contributor"
  principal_id         = azurerm_user_assigned_identity.managed-id.principal_id
}

resource "azurerm_role_assignment" "aks-mi-roles-aks-2-rt" {
  scope                = azurerm_route_table.aks-2-rt.id
  role_definition_name = "Network Contributor"
  principal_id         = azurerm_user_assigned_identity.managed-id.principal_id
}
